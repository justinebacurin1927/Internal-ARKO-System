...
function WeekView({weekDays,sprint,events,onCellClick,onEventClick,isInSprint}:{
  ...
  return (
    <div className="flex-1 rounded-lg ring-1 ring-black/[0.06] bg-white overflow-hidden relative flex flex-col">
      {/* Day header row */}
      <div className="flex h-[38px] shrink-0 border-b border-border-subtle">
        <div className="w-[44px] shrink-0" />
        {weekDays.map((d,i)=>(
          <div key={i} className={`flex-1 flex flex-col items-center justify-center ${i<6?'border-r':''} ${d.today?'bg-accent-50':isInSprint(d.ds)?'bg-accent-500/[0.04]':''}`}
            style={isInSprint(d.ds)&&sprint?{borderTop:`3px solid ${sprint.color}`}:{}}>
...
function MonthView({days,eventsByDate,onDayClick,isInSprint}:{
  ...
  return (
    <div className="flex-1 rounded-lg ring-1 ring-black/[0.06] bg-white overflow-hidden flex flex-col">
      {/* Day headers */}
      <div className="flex shrink-0 border-b border-border-subtle">
        {DAY_SHORT.map((a,i)=>(
          <div key={i} className="flex-1 h-[30px] flex items-center justify-center">
...
function YearView({focusYear,eventsByDate,onMonthClick}:{
  ...
  return (
    <div className="flex-1 rounded-lg ring-1 ring-black/[0.06] bg-white p-5 overflow-y-auto">
