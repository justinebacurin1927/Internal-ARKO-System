...
const PerformanceChart = memo(function PerformanceChart({
  ...
  // Monthly net = income - expense per month
  // Growth rate = (net_t - net_t-1) / |net_t-1| * 100
  ...
  // Y-axis range: auto with -1/+4 default
  const rawMin = Math.min(0, ...plotRates)
  const rawMax = Math.max(0, ...plotRates)
  const dataMin = rawMin >= 0 ? -1 : rawMin - Math.max(Math.abs(rawMin) * 0.2, 0.5)
  const dataMax = rawMax <= 0 ? 4 : rawMax + Math.max(rawMax * 0.2, 0.5)
    ...
    <div className="relative w-full h-full select-none">
      ...
      >
        ...
        {pts.map((pt, i) => {
          ...
          return (
            <g key={i}>
              {/* Active: target rings + anchor line */}
              {active && (
                <>
                  <circle
      ...
      {/* ══ Absolute-positioned tooltip (DOM, not SVG) — avoids foreignObject issues ═══ */}
      {hoverPt && (
        <div
      ...
      {/* ══ KPI chips (top-right) ═══ */}
      <div
        className="absolute top-0 right-0 flex items-center gap-1.5 pointer-events-none"
      >
        <div
